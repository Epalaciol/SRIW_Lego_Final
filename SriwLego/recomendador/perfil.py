...
def actualizar_perfil(usuario):
    ...
    if(calificaciones.count() < 5):
        print("no se cambio el perfil, aun no tiene suficientes calificaciones")
    else:
...
def crear_perfil (usuario,architecture,city,friends,batman,minecraft,precio,nPiezas): 
    try:
        ...
        for pro in productos:
            pm = pm.append({"Precio":pro.precio, "nPiezas":pro.nPiezas},ignore_index=True)
        precio = ((precio-pm.Precio.min())/(pm.Precio.max()-pm.Precio.min()))
        if(precio >1):
            precio=1
        nPiezas = ((nPiezas-pm.nPiezas.min())/(pm.nPiezas.max()-pm.nPiezas.min()))
        if(nPiezas >1):
            nPiezas=1
